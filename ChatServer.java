





import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;
import java.io.PrintWriter;
import java.net.ServerSocket;
import java.net.Socket;
import java.util.HashSet;

/**
 * A multithreaded chat room server.  When a client connects the
 * server requests a screen name by sending the client the
 * text "SUBMITNAME", and keeps requesting a name until
 * a unique one is received.  After a client submits a unique
 * name, the server acknowledges with "NAMEACCEPTED".  Then
 * all messages from that client will be broadcast to all other
 * clients that have submitted a unique screen name.  The
 * broadcast messages are prefixed with "MESSAGE ".
 *
 * Because this is just a teaching example to illustrate a simple
 * chat server, there are a few features that have been left out.
 * Two are very useful and belong in production code:
 *
 *     1. The protocol should be enhanced so that the client can
 *        send clean disconnect messages to the server.
 *
 *     2. The server should do some logging.
 *     
 *-----------------------------------------------------------------
 *     		Algorithm
 *     
 *     1.	Create the Socket
 *     2.	Bind the Socket to a specified port
 *     3.	
 *     
 *-----------------------------------------------------------------

 */
public class ChatServer {

    /**
     * Port Declaration (That the Server Listens on).
     */
    private static final int PORT = 9001;

    /**
     * The Set of all unique client usernames in the chat room.  Maintained
     * to verify if a clients username is unique.
     */
    private static HashSet<String> names = new HashSet<String>();

    /**
     * The Set of all of the print writers for all clients.
     */
    private static HashSet<PrintWriter> writers = new HashSet<PrintWriter>();

    /**
     * The application's main method, which listens on a port and
     * spawns handler threads.
     */
    public static void main(String[] args) throws Exception {
        System.out.println("The chat server is running.");
        
        /**
         * Socket Declaration
         * ServerSocket(PORT): Creates a server socket, bound to the specified port.
         */
        ServerSocket listener = new ServerSocket(PORT);
        
        /**
         * Infinite Loop
         */
        try {
			/**
			 * accept()		Listens for a connection to be made to this socket and accepts it.
	         * start()		Causes this thread to begin execution.
	         * ThreadHandler()	Constructs a thread handler.	
			 */
            while (true) {
                new ThreadHandler(listener.accept()).start();
            }
            //close()		Closes this socket.
        } finally {
            listener.close();
        }
    }

    /**
     * ThreadHandler class.  Handlers are spawned from the listening
     * loop and are responsible for a dealing with a single client
     * and broadcasting its messages.
     */
    private static class ThreadHandler extends Thread {
        private String name;
        private Socket socket;
        private BufferedReader in;
        private PrintWriter out;

        /**
         * Constructs a handler thread, squirreling() away the socket.
         * All the interesting work is done in the run method.
         */
        public ThreadHandler(Socket socket) {
            this.socket = socket;
        }

        /**
         * Services this thread's client by requesting a
         * screen name until a unique one has been submitted, then
         * acknowledges the name and registers the output stream for
         * the client in a global set, then repeatedly gets inputs and
         * broadcasts them.
         */
        public void run() {
            try {

            	/**
            	 * Create input character stream for the socket.
            	 * getInputStream()		Returns an input stream for this socket.
            	 * InputStreamReader()	Creates an InputStreamReader that uses the default charset.
            	 * BufferedReader()		Creates a buffering character-input stream that uses a default-sized input buffer.
            	 */
                in = new BufferedReader(new InputStreamReader(
                    socket.getInputStream()));
                
                /**
                 * Create output character stream for the socket.
                 * getOutputStream()	Returns an output stream for this socket.
                 * PrintWriter()		Creates a new PrintWriter from an existing OutputStream.
                 */
                out = new PrintWriter(socket.getOutputStream(), true);
                
                /**
                 * Request a name from this client.
                 * Keep requesting until a name is submitted that is not already used.
                 * Note that checking for the existence of a name and adding the name 
                 * must be done while locking the set of names.
                 */
                while (true) {
                    out.println("SUBMITNAME");
                    name = in.readLine();
                    if (name == null) {
                        return;
                    }
                    synchronized (names) {
                        if (!names.contains(name)) {
                            names.add(name);
                            break;
                        }
                    }
                }

                /**
                 *Now that a successful name has been chosen, add the
                 *socket's print writer to the set of all writers so
                 *this client can receive broadcast messages.
                 */
                out.println("NAMEACCEPTED");
                writers.add(out);

                /**
                 * Accept messages from this client and broadcast them.
                 * Ignore other clients that cannot be broadcasted to.
                 */
                while (true) {
                	//readLine()	Reads a line of text.
                    String input = in.readLine();
                    if (input == null) {
                        return;
                    }
                    for (PrintWriter writer : writers) {
                        writer.println("MESSAGE " + name + ": " + input);
                    }
                }
            } catch (IOException e) {
                System.out.println(e);
            } finally {
                // This client is going down!  Remove its name and its print
                // writer from the sets, and close its socket.
                if (name != null) {
                    names.remove(name);
                }
                if (out != null) {
                    writers.remove(out);
                }
                try {
                    socket.close();
                } catch (IOException e) {
                }
            }
        }
    }
}